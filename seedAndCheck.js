...
async function seed() {
    ...
    try {
        ...
        const check = await supabase.from('eventos').select('id').limit(1);
        if (check.data && check.data.length > 0) {
            console.log("Table 'eventos' is NOT empty. Skipping seed.");
            return;
        }

        for (const evento of EVENTOS_DATA) {
            const { data, error } = await supabase.from('eventos').insert([evento]);
            if (error) {
                console.error("Error inserting:", evento.nome, error.message);
            } else {
                console.log("Inserted:", evento.nome);
            }
        }
    } catch (err) {
        console.error(err);
...
async function checkParticipantes() {
    console.log("Checking 'participantes' table...");
    try {
        const { data, error } = await supabase.from('participantes').select('*').limit(1);
        if (error) {
            console.error("Error accessing 'participantes' table:", error);
        } else {
            console.log("'participantes' table exists and is accessible.");
        }
    } catch (e) {
        console.error(e);
    }
